#!/usr/bin/env python3
"""Main page template for BiliRSS"""
...
/* Collection Card */
.col-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 10px; padding: 16px; }
...
.col-card .col-desc { font-size: 12px; color: var(--text3); margin-bottom: 10px; }
.col-card .col-items { font-size: 12px; color: var(--text2); }
.col-card .col-actions { display: flex; gap: 6px; }
  ...
  <div class="modal-overlay" id="newColModal">
    <div class="modal">
      ...
      <form onsubmit="return submitCollection(event)">
        ...
        <div class="form-group">
          <label>合集描述</label>
          <textarea id="col_desc" placeholder="可选"></textarea>
        </div>
...
async function deleteCollection(id) {
  ...
  showToast('合集已删除'); setTimeout(() => location.reload(), 800);
}
