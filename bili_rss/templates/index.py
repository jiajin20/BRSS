#!/usr/bin/env python3
"""Main page template for BiliRSS v5
Fix: Move modals outside tab-content divs so they work from any tab
Fix: Reduce stats refresh interval to 5s, immediate refresh on load
"""
...
.audio-card { background: var(--bg2); border: 1px solid var(--border); border-radius: 10px; padding: 14px; transition: border-color 0.2s, box-shadow 0.2s; position: relative; }
.audio-card:hover { border-color: var(--accent); }
.audio-card.selected { border-color: var(--accent); box-shadow: 0 0 0 2px rgba(122,162,247,0.3); }
...
.audio-card .card-checkbox { position: absolute; top: 8px; left: 8px; width: 20px; height: 20px; accent-color: var(--accent); cursor: pointer; z-index: 2; opacity: 0.7; }
.audio-card .card-checkbox:hover { opacity: 1; }
.audio-card.selected .card-checkbox { opacity: 1; }
...
/* Modal */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 1000; align-items: center; justify-content: center; display: none; }
...
/* Batch action bar */
.batch-bar { position: sticky; bottom: 0; left: 0; right: 0; background: var(--card); border-top: 1px solid var(--border); padding: 12px 20px; display: flex; align-items: center; gap: 12px; z-index: 100; transform: translateY(100%); transition: transform 0.3s; border-radius: var(--radius) var(--radius) 0 0; }
.batch-bar.show { transform: translateY(0); }
.batch-bar .batch-info { flex: 1; font-size: 14px; color: var(--text2); }
.batch-bar .batch-info strong { color: var(--accent); }
...
<div class="header">
  <div class="header-inner">
    ...
    <div class="header-stats">
      <div class="stat-box">
        <div class="num" id="stat-audio">{{ total_audio }}</div>
        <div class="label">音频文件</div>
      </div>
      <div class="stat-box">
        <div class="num" id="stat-size">{{ total_size_str }}</div>
        <div class="label">总大小</div>
      </div>
      <div class="stat-box">
        <div class="num" id="stat-categories">{{ categories|length }}</div>
        <div class="label">分类</div>
      </div>
      <div class="stat-box">
        <div class="num" id="stat-collections">{{ total_collections }}</div>
  ...
  <div class="server-status-bar">
    <div class="status-card">
      ...
      <div class="sc-info">
        ...
        <div class="sc-value accent" id="stat-disk">{{ server_status.disk_used }} / {{ server_status.disk_total }}</div>
        <div class="disk-bar"><div class="fill {% if server_status.disk_percent < 70 %}green{% elif server_status.disk_percent < 90 %}yellow{% else %}red{% endif %}" id="stat-disk-fill" style="width:{{ server_status.disk_percent }}%"></div></div>
        <div style="font-size:11px;color:var(--text3);margin-top:2px" id="stat-disk-detail">剩余 {{ server_status.disk_free }}（{{ server_status.disk_percent }}% 已用）</div>
      </div>
    </div>
    <div class="status-card">
      <div class="sc-icon">🎵</div>
      <div class="sc-info">
        ...
        <div class="sc-value green" id="stat-audio-detail">{{ total_audio }} 首音频 · {{ total_collections }} 个合集</div>
      </div>
    </div>
    <div class="status-card" id="uptime-app" v-cloak data-start-time="{{ server_status.start_time }}">
      <div class="sc-icon">⏱️</div>
      <div class="sc-info">
        <div class="sc-label">服务运行时长</div>
        <div class="sc-value cyan" v-text="uptimeText"></div>
      </div>
    </div>
  </div>
</div>
...
<!-- Tab Navigation -->
<div class="tabs">
  <button class="tab-btn active" data-tab="download" onclick="switchTab('download')">📥 下载管理</button>
  <button class="tab-btn" data-tab="files" onclick="switchTab('files')">📁 文件管理</button>
  <button class="tab-btn" data-tab="collections" onclick="switchTab('collections')">🎵 合集管理</button>
  <button class="tab-btn" data-tab="rss" onclick="switchTab('rss')">📡 RSS 订阅</button>
</div>
...
<div class="tab-content" id="tab-files">
  ...
  <div class="audio-grid" id="audioGrid">
    ...
    <div class="audio-card" data-bvid="{{ audio.bv_id }}" data-title="{{ audio.title|lower }}" data-uploader="{{ audio.uploader|lower }}" data-format="{{ audio.format }}">
      <input type="checkbox" class="card-checkbox" data-bvid="{{ audio.bv_id }}" onchange="onCardCheckChange()">
      {% if audio.thumbnail %}
      ...
      <div class="meta" style="margin-top:4px">
        ...
        <span style="margin-left:auto">
          ...
          <a href="#" onclick="deleteSingleAudio('{{ audio.bv_id }}');return false" style="font-size:12px;color:var(--red);margin-left:8px">删除</a>
        </span>
      </div>
    </div>
    {% endfor %}
  ...
  <div class="batch-bar" id="batchBar">
    <div class="batch-info">已选择 <strong id="selectedCount">0</strong> 个文件</div>
    <div class="batch-actions">
      <button class="btn btn-ghost btn-sm" onclick="cancelSelection()">取消选择</button>
      <button class="btn btn-sm btn-accent2" onclick="batchAddToCollection()">加入合集</button>
...
<!-- ========== GLOBAL MODALS (outside tabs so they work from any tab) ========== -->
...
<!-- Delete Collection with Password Modal -->
...
<!-- Delete Key Modal (for audio file deletion - works from ANY tab) -->
<div class="modal-overlay" id="deleteKeyModal">
  <div class="modal">
    <button class="modal-close" onclick="hideModal('deleteKeyModal')">×</button>
    <h3>🔑 验证删除密钥</h3>
    <div class="password-hint">
      <span>⚠️</span>
      <span>删除文件需要管理员密钥验证，请输入密钥后确认</span>
    </div>
    <form onsubmit="return confirmDeleteWithKey(event)">
      <div class="form-group">
        <label>删除密钥</label>
        <input type="password" id="delete_key_input" placeholder="请输入管理员密钥" required>
      </div>
      <input type="hidden" id="delete_target_type">
      <input type="hidden" id="delete_target_ids">
      <div style="display:flex;gap:8px">
        <button type="button" class="btn btn-ghost" style="flex:1" onclick="hideModal('deleteKeyModal')">取消</button>
        <button type="submit" class="btn btn-danger" style="flex:1">确认删除</button>
      </div>
    </form>
  </div>
...
<!-- Add to Collection Modal (works from ANY tab) -->
  ...
  <div class="footer-inner">
    <div class="footer-left">
      ...
      <div class="footer-text">
        ...
        开发：<a href="https://github.com/jiajin20" target="_blank">不正经<br></a>
        GitHub：<a href="https://github.com/jiajin20" target="_blank">github.com/jiajin20<br></a>
        邮箱：<a href="mailto:jiajinol@foxmail.com" target="_blank">jiajinol@foxmail.com</a>
      </div>
    </div>
    <div class="footer-right">
      <div class="footer-links">
        <a href="https://github.com/jiajin20" target="_blank">GitHub</a>
        <span style="color:var(--text3)">© 2026 BiliRSS</span>
      </div>
    </div>
  </div>
</div>
...
const TAB_NAMES = ['download', 'files', 'collections', 'rss'];
...
function switchTab(name) {
  ...
  sessionStorage.setItem('bilirss_tab', name);
}

function restoreTab() {
  const saved = sessionStorage.getItem('bilirss_tab');
...
// === Smart Reload: preserves current tab ===
function smartReload() {
  // Tab state already saved in sessionStorage via switchTab
  location.reload();
}
...
async function submitTask(e) {
  ...
  if (data.ok) { showToast('下载任务已创建！'); setTimeout(smartReload, 1500); }
...
async function submitCategory(e) {
  ...
  const data = await resp.json();
  if (data.ok) { showToast('分类已创建！'); setTimeout(smartReload, 800); }
  else showToast(data.error || '创建失败', 'error');
  return false;
}
...
async function submitCollection(e) {
  ...
  const data = await resp.json();
  if (data.ok) { hideModal('newColModal'); showToast('合集已创建！'); setTimeout(smartReload, 800); }
  else showToast(data.error || '创建失败', 'error');
  return false;
}
...
async function deleteCategory(id) {
  ...
  showToast('分类已删除'); setTimeout(smartReload, 800);
}
async function deleteTask(id) {
  if (!confirm('确定要删除此任务吗？')) return;
  await fetch('/api/task/' + id, { method: 'DELETE' });
  showToast('任务已删除'); setTimeout(smartReload, 800);
}

// === Delete Audio with Secret Key ===
async function deleteSingleAudio(bvId) {
  if (!confirm('确定要删除此音频文件吗？')) return;
  // Check if we have a cached key
  const cachedKey = sessionStorage.getItem('bilirss_delete_key');
  if (cachedKey) {
    const form = new FormData();
    form.append('secret_key', cachedKey);
    const resp = await fetch('/api/audio/' + bvId, { method: 'DELETE', body: form });
    const data = await resp.json();
    if (data.ok) {
      showToast('音频已删除');
      removeAudioCardFromDOM(bvId);
      refreshStats();
    } else if (data.need_key || data.error === '密钥错误') {
      // Cached key is invalid, clear and re-ask
      sessionStorage.removeItem('bilirss_delete_key');
      showDeleteKeyModal('single', bvId);
    } else {
      showToast(data.error || '删除失败', 'error');
    }
  } else {
...
function showDeleteKeyModal(type, ids) {
  document.getElementById('delete_target_type').value = type;
  document.getElementById('delete_target_ids').value = ids;
  document.getElementById('delete_key_input').value = '';
  showModal('deleteKeyModal');
}

async function confirmDeleteWithKey(e) {
  e.preventDefault();
  const key = document.getElementById('delete_key_input').value;
  const type = document.getElementById('delete_target_type').value;
  const ids = document.getElementById('delete_target_ids').value;

  if (type === 'single') {
    const form = new FormData();
    form.append('secret_key', key);
    const resp = await fetch('/api/audio/' + ids, { method: 'DELETE', body: form });
    const data = await resp.json();
    if (data.ok) {
      sessionStorage.setItem('bilirss_delete_key', key);
      hideModal('deleteKeyModal');
      showToast('音频已删除');
      removeAudioCardFromDOM(ids);
      refreshStats();
    } else {
      showToast(data.error || '密钥错误', 'error');
    }
  } else if (type === 'batch') {
    const form = new FormData();
    form.append('secret_key', key);
    form.append('bv_ids', ids);
    const resp = await fetch('/api/audio/batch-delete', { method: 'POST', body: form });
    const data = await resp.json();
    if (data.ok) {
      sessionStorage.setItem('bilirss_delete_key', key);
      hideModal('deleteKeyModal');
      showToast('已删除 ' + data.deleted + ' 个文件');
      // Remove cards from DOM
      ids.split(',').forEach(bvId => removeAudioCardFromDOM(bvId.trim()));
      cancelSelection();
      refreshStats();
    } else {
      showToast(data.error || '密钥错误', 'error');
...
function removeAudioCardFromDOM(bvId) {
  const card = document.querySelector('.audio-card[data-bvid="' + bvId + '"]');
  if (card) {
...
function toggleSelectAll() {
  ...
  document.querySelectorAll('.audio-card .card-checkbox').forEach(cb => {
    const card = cb.closest('.audio-card');
...
function onCardCheckChange() {
  const checkboxes = document.querySelectorAll('.audio-card .card-checkbox');
...
function updateBatchBar() {
  const selected = document.querySelectorAll('.audio-card .card-checkbox:checked');
...
function cancelSelection() {
  document.querySelectorAll('.audio-card .card-checkbox').forEach(cb => {
    cb.checked = false;
    cb.closest('.audio-card').classList.remove('selected');
  });
...
function getSelectedBvIds() {
  return [...document.querySelectorAll('.audio-card .card-checkbox:checked')]
    .map(cb => cb.dataset.bvid)
    .filter(Boolean);
}

// === Batch Delete ===
function showBatchDeleteModal() {
  const ids = getSelectedBvIds();
  if (!ids.length) { showToast('请先选择文件', 'error'); return; }

  const cachedKey = sessionStorage.getItem('bilirss_delete_key');
  if (cachedKey) {
    doBatchDelete(cachedKey, ids);
  } else {
    showDeleteKeyModal('batch', ids.join(','));
  }
}

async function doBatchDelete(key, ids) {
  const form = new FormData();
  form.append('secret_key', key);
  form.append('bv_ids', ids.join(','));
  const resp = await fetch('/api/audio/batch-delete', { method: 'POST', body: form });
  const data = await resp.json();
  if (data.ok) {
    showToast('已删除 ' + data.deleted + ' 个文件');
    ids.forEach(bvId => removeAudioCardFromDOM(bvId));
    cancelSelection();
    refreshStats();
  } else if (data.error === '密钥错误') {
    sessionStorage.removeItem('bilirss_delete_key');
    showDeleteKeyModal('batch', ids.join(','));
  } else {
    showToast(data.error || '删除失败', 'error');
...
function batchAddToCollection() {
  ...
  if (!ids.length) { showToast('请先选择文件', 'error'); return; }
  pendingBvIds = ids;
  document.getElementById('addColBvIds').textContent = ids.join(', ');
...
async function refreshStats() {
  try {
    ...
    if (audioEl) audioEl.textContent = s.total_audio;
    if (sizeEl) sizeEl.textContent = s.total_size_str;
    if (catEl) catEl.textContent = s.total_categories;
    if (colEl) colEl.textContent = s.total_collections;
    // Update server status
    const diskEl = document.getElementById('stat-disk');
    const diskFillEl = document.getElementById('stat-disk-fill');
    const diskDetailEl = document.getElementById('stat-disk-detail');
    const audioDetailEl = document.getElementById('stat-audio-detail');
    if (diskEl) diskEl.textContent = s.disk_used + ' / ' + s.disk_total;
    if (diskFillEl) {
      diskFillEl.style.width = s.disk_percent + '%';
      diskFillEl.className = 'fill ' + (s.disk_percent < 70 ? 'green' : s.disk_percent < 90 ? 'yellow' : 'red');
    }
    if (diskDetailEl) diskDetailEl.textContent = '剩余 ' + s.disk_free + '（' + s.disk_percent + '% 已用）';
    if (audioDetailEl) audioDetailEl.textContent = s.total_audio + ' 首音频 · ' + s.total_collections + ' 个合集';
  } catch(e) {
    // silently fail
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Immediately refresh stats on page load for real-time display
  refreshStats();
...
// === Vue: Real-time Uptime Counter ===
Vue.createApp({
  data() {
    return { uptimeText: '', startTime: 0, timer: null, calTimer: null };
  },
  mounted() {
    const el = document.getElementById('uptime-app');
    if (el) {
      this.startTime = parseFloat(el.dataset.startTime) || 0;
      this.tick();
      this.timer = setInterval(() => this.tick(), 1000);
      // 每 3 分钟从接口校准，防止客户端时钟漂移
      this.calTimer = setInterval(() => this.calibrate(), 180000);
    }
  },
  beforeUnmount() {
  ...
  methods: {
    tick() {
      const s = Math.floor(Date.now() / 1000 - this.startTime);
      if (s < 0) { this.uptimeText = '0分 0秒'; return; }
      const d = Math.floor(s / 86400);
      const h = Math.floor((s % 86400) / 3600);
      const m = Math.floor((s % 3600) / 60);
      const sec = s % 60;
      if (d > 0) this.uptimeText = `${d}天 ${h}小时 ${m}分 ${sec}秒`;
      else if (h > 0) this.uptimeText = `${h}小时 ${m}分 ${sec}秒`;
      else this.uptimeText = `${m}分 ${sec}秒`;
    },
    async calibrate() {
      try {
